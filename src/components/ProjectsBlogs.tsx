...
export const ProjectAdapter = ({imageSrc,title,stack}: ProjectAdapterProps ) => {
  return (
    <>
      <div className="relative h-20 w-32 md:h-[250px] md:w-[300px]">
        <Image alt={"logo"} src={imageSrc} fill className="rounded-lg opacity-90"/>
      </div>
      <div className="flex flex-col justify-center pl-4 md:pl-12 lg:pl-16 md:space-y-2">
        <div className="flex gap-2 text-xs md:text-sm text-zinc-500 dark:text-lime-100">
          {stack.map((tech, index) => (
            <p key={index}>{tech}</p>
          ))}
        </div>
        <h2 className="text-sm md:text-3xl dark:text-lime-100">{title}</h2>
      </div>
    </>
  )
}
